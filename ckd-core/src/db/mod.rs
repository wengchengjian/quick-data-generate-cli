use sqlx::{
    pool::PoolConnection, sqlite::SqliteConnectOptions, Executor, Pool, Sqlite, SqliteConnection,
    SqlitePool,
};
...
pub async fn init(conn: &mut SqliteConnection) -> crate::Result<()> {
    ...
    // 建索引
    conn.execute("CREATE UNIQUE INDEX IF NOT EXISTS schema_unq_name on data_source_schema (name)")
        .await?;
...
mod tests {
    ...
    fn test_create_db() {
        block_on(async {
            let _ = create_pool().await.unwrap();
        });
    }
    #[test]
