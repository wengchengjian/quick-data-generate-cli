...
use crate::task::kafka::KafkaTask;
use crate::task::Task;
use crate::Json;
...
impl KafkaDataSource {
    ...
    pub async fn get_provider(&self) -> crate::Result<FutureProducer> {
    ...
    pub async fn get_consumer(
        ...
        group: String,
        topics: &[&str],
    ) -> crate::Result<StreamConsumer<DefaultConsumerContext>> {
        let host = format!("{}:{}", self.host().await, self.port().await);
